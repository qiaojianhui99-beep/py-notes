...
**说明**：单元素元组必须在元素后加逗号，否则会被当作普通的括号表达式。
...
```python
position = (10, 20, 30)
x, y = position  # ValueError: too many values to unpack
```
...
# 方法 2：使用扩展解包
x, y, *rest = position  # x=10, y=20, rest=[30]
```

**说明**：解包时变量数量必须与元组元素数量匹配，或使用 `*` 捕获剩余元素。
...
# 但元组内的可变对象可以修改
t[0].append(3)  # 正确：可以修改列表内容
print(t)  # ([1, 2, 3], [3, 4])
```

**说明**：元组的不可变是指元组结构不变，但元素如果是可变对象（如列表），元素内容可以修改。
